...
import type { CredentialAttributeType, CredentialDefinitionType } from '@/openapi-types'
...
export const DisplayAddedCredentials = ({ credentials, removeCredential }: DisplayAddedCredentialsProps) => {
  ...
  const handleAttributeChange = (credentialId: string, attrIndex: number, newValue: string) => {
    setLocalAttributes((prev) => ({
      ...prev,
      [credentialId]:
        prev[credentialId]?.map(({ id, ...attr }, i) => (i === attrIndex ? { ...attr, value: newValue } : attr)) || [],
    }))
  }
  ...
  useEffect(() => {
    ...
    credentials.forEach((credential: CredentialDefinitionType) => {
      initialAttributes[credential.id] =
        credential?.credentialSchema?.attributes?.map((attr: CredentialAttributeType) => ({
          ...attr,
          value: attr.value || '',
        })) || []
    })
  ...
  const updateCredentialSchema = async (credentialData: any) => {
    try {
      const {
        credentialSchema: { id: credentialSchemaId },
      } = credentialData
              ...
              >
                ...
                <div className="flex items-center flex-1">
                  ...
                  <div className="space-y-1 ml-4">
                    ...
                    <p className="text-sm text-muted-foreground">{credential?.issuer_name ?? 'Test college'}</p>
                  </div>
                </div>
